...
import { BarChart2, FileText, CalendarCheck, MessageSquare, Zap, Shield, Target, TrendingUp } from "lucide-react";
...
const features = [
  ...
  {
    icon: <CalendarCheck className="w-8 h-8 text-purple-600 dark:text-purple-400" />,
    name: "Predictive Analytics",
    desc: "Forecast emission trends and compliance risks with AI-powered models built for your business.",
    color: "from-purple-500 to-pink-500"
  },
  {
    icon: <MessageSquare className="w-8 h-8 text-orange-600 dark:text-orange-400" />,
...
const additionalFeatures = [
  { icon: <Zap />, title: "Lightning Fast", desc: "Reports generated in seconds" },
  { icon: <Shield />, title: "Enterprise Security", desc: "Bank-level encryption & compliance" },
  { icon: <Target />, title: "Goal Tracking", desc: "Monitor progress toward sustainability targets" },
  { icon: <TrendingUp />, title: "Performance Metrics", desc: "Real-time KPI monitoring" },
];

export const Features = () => (
  <section id="features" className="py-24 md:py-32 bg-white dark:bg-gray-900 relative overflow-hidden transition-colors duration-300">
    {/* Background Elements */}
    ...
    <div className="container relative">
      ...
      <div className="text-center mb-20">
        ...
        <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto leading-relaxed">
          Comprehensive ESG management tools designed to help your business thrive in a sustainable future
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-20">
        {features.map((feature, index) => (
          <div 
            key={feature.name} 
            className="group relative bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-8 shadow-lg dark:shadow-gray-900/20 hover:shadow-2xl dark:hover:shadow-gray-900/40 transition-all duration-500 hover:-translate-y-2 animate-fade-in backdrop-blur-sm"
            style={{ animationDelay: `${index * 100}ms` }}
          >
            {/* Gradient Background */}
            <div className={`absolute inset-0 bg-gradient-to-br ${feature.color} opacity-0 group-hover:opacity-5 dark:group-hover:opacity-10 rounded-2xl transition-opacity duration-500`}></div>
            
            {/* Icon Container */}
            <div className="relative mb-6">
              <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${feature.color} p-2 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300`}>
